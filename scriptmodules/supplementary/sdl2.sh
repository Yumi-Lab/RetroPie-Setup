...
function _list_depends_sdl2() {
    ...
    # Bookworm-compatible package names (fcitx-libs-dev → removed, transitional mesa names updated)
    ...
    ! isPlatform "vero4k" && depends+=(libx11-dev libxcursor-dev libxext-dev libxi-dev libxinerama-dev libxkbcommon-dev libxrandr-dev libxss-dev libxt-dev libxv-dev libxxf86vm-dev libgl-dev)
    isPlatform "gles" || isPlatform "gl" && depends+=(libegl-dev libgles-dev)
    isPlatform "gl" || isPlatform "rpi" && depends+=(libgl-dev libglu1-mesa-dev)
